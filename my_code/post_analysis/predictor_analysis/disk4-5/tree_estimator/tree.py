...
def get_medain(query_data):
    metrics = [sinlge_data["metric"] for sinlge_data in query_data]
    metrics.sort()
    return metrics[ ( len(metrics)/2 ) ]
...
class Tree(object):
    ...
    # query_data is the data structure of [{"day_qid":day_qid,"values": [], "metric":metric}]
    ...
    def compute_error(self,query_data,error_threshold):
        ...
        for sinlge_data in query_data:
            day_qid = sinlge_data["day_qid"]
            real_values[day_qid] = sinlge_data["metric"] 
