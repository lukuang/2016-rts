"""
plot the trend of different expansion method
vs. predicted silent day probability
"""
...
def main():
    ...
    parser.add_argument("--base_model","-bm",default="raw_f2exp")
        ...
        for year in datasets[collection_name].years:
            ...
            for day in datasets[collection_name]._prediction[year]:
                ...
                for qid in datasets[collection_name]._prediction[year][day]:
                    try:
                        base_performance = performances[args.base_model][qid][day_string]
                    except KeyError:
                        continue
                    else:
                        query_day_prob = datasets[collection_name].get_prob(year,day,qid)
                        if query_day_prob < 0.5:
                                continue
                        expansion_performance = performances[args.expansion_model][qid][day_string]
                        probs.append(query_day_prob)
                        # base.append(base_performance)
                        # expansion.append(expansion_performance)
                        diff.append(expansion_performance - base_performance)
            dest_file = os.path.join(args.dest_dir, "%s_%s_%s.png" %(year.name,
                                                                     args.base_model,
                                                                     args.expansion_model))
            plt.plot(probs, diff, 'ro',label=args.base_model)
            # plt.plot(probs, expansion, 'bx',label=args.expansion_model)
            plt.legend()
