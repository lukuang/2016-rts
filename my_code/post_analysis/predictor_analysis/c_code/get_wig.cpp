...
class Query{
    ...
    public:
        ...
        float compute_query_wig(indri::collection::Repository& r,vector<string>& document_results){
            indri::server::LocalQueryServer local(r);
            ...
            for(int i=0;i<document_results.size();i++){
                ...
                float term_lambda = 0.8/sqrt(term_count);
                float phrase_lambda = .0;
                if(phrase_count!=0){
                    phrase_lambda = 0.2/sqrt(phrase_count);
                }
...
map<string, float> get_wig(indri::collection::Repository& r,map<string, vector<string> >& results,map<string, vector<string> >& queries, const string& indexName, const bool& debug){
  ...
  for(map<string, vector<string> >::iterator it=queries.begin();it!=queries.end(); ++it ){
    ...
    float query_wig = query_with_phrases[qid].compute_query_wig(r,results[qid]);
    wig[qid] = query_wig;
...
static void usage( indri::api::Parameters param ) {
  if( !( param.exists( "query" ) )    || 
      !( param.exists( "index" ) ) ||
      !( param.exists( "result") ) ) {
   std::cerr << "get_wig usage: " << std::endl
             << "   get_wig -query=myquery -index=myindex -result=myresult" << std::endl;
   exit(-1);
  }
...
int main(int argc, char** argv){
    ...
    try {
        ...
        map<string, float> wig = get_wig(r,results,queries,rep_name,debug);
