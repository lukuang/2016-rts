"""
construct a tree estimator using all data from all years
"""
...
from plot_silentDay_predictor import R_DIR, Expansion,RetrievalMethod, SilentDaysFromJug
# R_DIR = {
#     Year.y2015:{},
#     Year.y2016:{}, 
#     Year.y2011:{} 
...
class EvalData(object):
    ...
    def __init__(self,year):
        ...
        if self._year == Year.y2015:
            ...
            self._prefix_jul = "201707"
            self._prefix_aug = "201708"
...
def main():
    ...
    for year in Year:
        ...
        eval_data = EvalData(year)
        result_dir = R_DIR[year][args.expansion][args.retrieval_method]
        results = read_results(result_dir,eval_data)
        query_data_file = os.path.join(args.tree_estimator_directory,year.name,args.expansion.name,args.retrieval_method.name)
        ...
        # create query_data
        # print eval_data.days
        for qid in eval_data.days:
            for day in eval_data.days[qid]:

                # skip silent days when constructing the performance 
                # prediction tree
